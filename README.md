...
* **Gestion automatisée de l'Inner-Loop (`run` & `push`) :** * Détecte automatiquement le port de votre serveur de développement local (Vite, Next, Angular, etc.) et met à jour dynamiquement le fichier `power.config.json`.
...
Ce projet est une extension VS Code complète (`.vsix`). Pour contourner les bugs de copie d'URL de l'interface GitHub, exécutez ce bloc de commandes segmenté dans votre terminal **PowerShell** :
