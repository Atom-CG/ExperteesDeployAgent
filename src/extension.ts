...
function obtenirOuCreerSession(threadId: string): SessionALM {
    if (!sessions.has(threadId)) {
        sessions.set(threadId, { etat: 'MENU_PRINCIPAL', nomProjet: '', frameworkChoisi: '', nomSolution: '', idSource: '', idCible: '', codeAppManquants: [], compteIndex: 0, environnementSelectionne: '' });
    }
...
function lireUrlServeur(): string | undefined {
    ...
    const configPath = path.join(workspaceFolders[0].uri.fsPath, 'power.config.json');
    ...
    try {
        ...
        if (typeof config.playUrl === 'string') { return config.playUrl; }
        ...
        if (typeof environmentId === 'string') {
            const connectionsPort: number = config.server?.port ?? config.port ?? 8080;
            ...
            return `https://apps.powerapps.com/play/e/${environmentId}/app/local` +
        ...
        // Fallback : port seul
        if (config.server) {
            if (typeof config.server.url === 'string') { return config.server.url; }
            if (typeof config.server.port === 'number' || typeof config.server.port === 'string') {
...
function detecterPortDevServeur(): { port: number; framework: string } {
    const workspaceFolders = vscode.workspace.workspaceFolders;
    if (!workspaceFolders || workspaceFolders.length === 0) { return { port: 3000, framework: 'inconnu' }; }

    const racine = workspaceFolders[0].uri.fsPath;

    // 1. Port explicite dans vite.config.* (priorité maximale)
    for (const nom of ['vite.config.ts', 'vite.config.js', 'vite.config.mts', 'vite.config.mjs']) {
        ...
        if (fs.existsSync(vitePath)) {
            try {
                ...
                if (match) { return { port: parseInt(match[1], 10), framework: 'Vite (port personnalisé)' }; }
            } catch { /* ignore */ }
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            if (deps['vue'] && (deps['vite'] || deps['@vitejs/plugin-vue'])) { return { port: 5173, framework: 'Vue + Vite' }; }
            if (deps['react'] && (deps['vite'] || deps['@vitejs/plugin-react'])) { return { port: 5173, framework: 'React + Vite' }; }
            if (deps['vite']) { return { port: 5173, framework: 'Vite' }; }
            if (deps['react-scripts']) { return { port: 3000, framework: 'React (CRA)' }; }
            if (deps['@angular/core']) { return { port: 4200, framework: 'Angular' }; }
            if (deps['next']) { return { port: 3000, framework: 'Next.js' }; }
        } catch { /* ignore */ }
    }
...
async function ouvrirNavigateurServeur(stream: vscode.ChatResponseStream): Promise<void> {
    const DELAI_DEMARRAGE_MS = 3000;
    await new Promise(resolve => setTimeout(resolve, DELAI_DEMARRAGE_MS));

    const url = lireUrlServeur();
    if (!url) {
        stream.markdown('> ⚠️ Impossible de trouver l\'URL du serveur dans `power.config.json`.\n');
        return;
    }
...
function sauvegarderEnvironnementDansConfig(envUrl: string): boolean {
    ...
    if (fs.existsSync(configPath)) {
        ...
        } catch { /* démarrer avec un objet vide */ }
    }
...
function afficherMenuCodeApp(stream: vscode.ChatResponseStream): void {
    ...
    stream.markdown('> Configurez votre projet actuel pour le connecter à Power Apps Code First.\n\n');
    stream.markdown('---\n\n');
    stream.markdown('| # | Action | Description |\n');
    stream.markdown('|---|--------|-------------|\n');
    stream.markdown('| **1** | `prérequis` | Vérifier et installer les prérequis |\n');
    stream.markdown('| **2** | `initialiser` | Initialiser directement le projet |\n\n');
...
async function verifierPrerequisCodeApp(
    ...
    const ppTools = vscode.extensions.getExtension('microsoft-IsvExpTools.powerplatform-vscode');
    if (ppTools) {
    ...
    stream.markdown('> _Un script de vérification est lancé dans le terminal. Consultez-le pour les résultats._\n\n');
    ...
    for (const item of session.codeAppManquants) {
        if (item.startsWith('ext:')) {
            ...
            try {
                ...
                stream.markdown(`⚠️ Impossible d'installer **${extId}** automatiquement. Installez-la via le Marketplace VS Code.\n\n`);
            }
        }
    }

    // Installation du SDK npm
    stream.markdown('📦 Installation du SDK **@microsoft/power-apps** dans le terminal...\n\n');
...
async function gererRequete(
    ...
    switch (session.etat) {
        ...
        case 'MENU_PRINCIPAL': {
            if (!messageUtilisateur || messageNormalise === 'menu' || messageNormalise === 'reset') {
            ...
            if (messageNormalise === '1' || messageNormalise === 'init') {
                ...
                stream.markdown('|--------|-------------|\n');
                stream.markdown('| **Framework** | Vue 3.5 (`<script setup lang="ts">`) |\n');
            ...
            if (messageNormalise === '3' || messageNormalise === 'run') {
                ...
                stream.markdown('▶️ **Lancement du proxy Power Apps et du serveur local...**\n\n');
                if (localAppUrlMisAJour) {
                    stream.markdown(`> 🔧 Framework détecté : **${framework}** — \`localAppUrl\` mis à jour → \`http://localhost:${portDev}\` dans \`power.config.json\`\n\n`);
                }
                executerDansTerminal('npm run dev & npx power-apps run');
                stream.markdown('> ⏳ Les deux serveurs démarrent en parallèle.\n\n');
                stream.markdown(`> ℹ️ L'avertissement _"NOT currently running"_ est **normal** — le serveur (port ${portDev}) prend quelques secondes à démarrer.\n\n`);
                stream.markdown(`> ⚠️ **Si le message persiste après 30 secondes**, vérifiez que votre config expose bien le port ${portDev} :\n`);
                stream.markdown(`> \`\`\`js\n> export default {\n>   server: { port: ${portDev} }\n> }\n> \`\`\`\n\n`);
            ...
            if (messageNormalise === '4' || messageNormalise === 'push') {
                ...
                stream.markdown('> Cette opération va **compiler le projet** (`npm run build`) puis **pousser le code** vers votre environnement Power Platform via `npx power-apps push`.\n\n');
                stream.markdown('**Confirmez-vous le push ?**\n\n');
                stream.markdown('> Répondez **oui** pour lancer le push, ou **non** pour revenir au menu principal.\n');
                break;
            }
            ...
            if (messageNormalise === '5' || messageNormalise === 'connexion') {
                ...
                stream.markdown('> La liste de vos profils PAC s\'affiche dans le terminal ci-dessous.\n\n');
                executerDansTerminal('pac auth list');
                stream.markdown('---\n\n');
                stream.markdown('| Saisie | Action |\n');
                stream.markdown('|--------|--------|\n');
                stream.markdown('| `N°` | Utiliser le compte correspondant (ex : `1`) |\n');
                stream.markdown('| `0` | Ajouter un nouveau compte |\n');
                stream.markdown('| `menu` | Revenir au menu principal |\n\n');
                stream.markdown('> ⬆️ Consultez le terminal pour voir vos comptes disponibles, puis répondez ici.\n');
                break;
            }
        ...
        case 'INIT_CHOIX_FRAMEWORK': {
            ...
            if (messageNormalise === '0' || messageNormalise === 'menu' || messageNormalise === 'retour') {
                reinitialiserSession(threadId);
                afficherMenuPrincipal(stream);
                break;
            }

            if (messageNormalise === '1' || messageNormalise === 'vue' || messageNormalise === 'continuer') {
                ...
                stream.markdown('❓ Choix non reconnu. Tapez `1` (Vue), `2` (React), `3` (Angular) ou `0` (menu).\n');
                break;
            }

            const labelsFramework: Record<string, string> = { vue: 'Vue 3.5', react: 'React 19', angular: 'Angular 19' };
            stream.markdown(`✅ Framework sélectionné : **${labelsFramework[session.frameworkChoisi]}**\n\n`);
            session.etat = 'INIT_NOM_PROJET';
            stream.markdown('Quel est le **nom du projet** ?\n> _Ex: mon-projet-pcf_');
            break;
        }
        ...
        case 'INIT_NOM_PROJET': {
            ...
            if (enDansDossier) {
                stream.markdown('> 📂 Installation dans le dossier de workspace actuel (pas de sous-dossier créé).\n\n');
                stream.markdown('> ⚠️ Si Vite demande comment gérer les fichiers existants, répondez directement dans le terminal.\n\n');
            }
            stream.markdown('> ⚠️ Si l\'initialisation pose des questions, suivez les invites dans le terminal.\n\n');
        ...
        case 'EXPORT_NOM_SOLUTION': {
            ...
            stream.markdown('Quel est l\'**ID de l\'environnement source** (où se trouve l\'application) ?\n');
            stream.markdown('> _(Cela permet de rafraîchir votre authentification MFA avant l\'export)_');
            break;
        }
        ...
        case 'EXPORT_ID_SOURCE': {
            ...
            stream.markdown('Le terminal gère l\'export. Pendant ce temps, voici comment remplir le fichier qui va s\'ouvrir :\n\n');
            
            // Le tutoriel qui remplace les commentaires JSON
            stream.markdown('### 📖 Comment remplir le fichier `deploymentsettings.json` ?\n');
            stream.markdown('1. Allez sur `make.powerapps.com` > **Environnement Cible** > **Connexions**.\n');
            stream.markdown('2. Cliquez sur chaque connexion requise (Dataverse, O365, etc.).\n');
            stream.markdown('3. Copiez l\'ID (le GUID) situé à la toute fin de l\'URL de votre navigateur.\n');
            stream.markdown('4. Collez cet ID dans le champ `"ConnectionId": ""` correspondant.\n');
            stream.markdown('5. Sauvegardez le fichier (`Ctrl+S`), puis tapez `@Expertees-Deploy 4`.\n\n');
        ...
        case 'IMPORT_ID_CIBLE': {
            ...
            stream.markdown(`🔥 **Lancement du déploiement vers ${session.idCible} !**\n\n`);
        ...
        case 'CODE_APP_PUSH_CONFIRM': {
            ...
            if (messageNormalise === 'non' || messageNormalise === 'n' || messageNormalise === 'no') {
            ...
            stream.markdown('❓ Répondez **oui** pour confirmer le push, ou **non** pour revenir au menu.\n');
            break;
        }
        ...
        case 'CODE_APP_MENU': {
            ...
            if (messageNormalise === '1' || messageNormalise === 'prérequis' || messageNormalise === 'prerequis') {
                await verifierPrerequisCodeApp(stream, session);
                stream.markdown('---\n\n');

                if (session.codeAppManquants.length === 0) {
                    session.etat = 'CODE_APP_INIT_CONFIRM';
                    stream.markdown('✅ **Toutes les extensions VS Code sont présentes !**\n\n');
                    stream.markdown('> Consultez le terminal pour les résultats Node.js, Git et SDK.\n\n');
                    stream.markdown('---\n\n**Souhaitez-vous initialiser ce projet en Power Apps Code App ?**\n\n');
                    stream.markdown('> Répondez **oui** pour continuer, ou **non** pour revenir au menu.\n');
                } else {
                    session.etat = 'CODE_APP_PREREQ_INSTALL';
                    stream.markdown('⚠️ **Des extensions VS Code sont manquantes.**\n\n');
                    stream.markdown('**Souhaitez-vous automatiser l\'installation des prérequis manquants ?**\n\n');
                    stream.markdown('> Répondez **oui** pour lancer l\'installation, ou **non** pour revenir au menu.\n');
                }
                break;
            }
            ...
            if (messageNormalise === '2' || messageNormalise === 'initialiser') {
                ...
                stream.markdown('Quel est l\'**ID de votre environnement Power Platform** ?\n\n');
        ...
        case 'CODE_APP_PREREQ_INSTALL': {
            if (messageNormalise === 'oui' || messageNormalise === 'o' || messageNormalise === 'yes') {
                ...
                session.etat = 'CODE_APP_INIT_CONFIRM';
                stream.markdown('\n---\n\n');
                stream.markdown('✅ **Installation terminée.** Vérifiez le terminal pour confirmer.\n\n');
                stream.markdown('**Souhaitez-vous initialiser ce projet en Power Apps Code App ?**\n\n');
                stream.markdown('> Répondez **oui** pour démarrer la configuration guidée, ou **non** pour revenir au menu.\n');
                break;
            }
            ...
            stream.markdown('❓ Répondez **oui** pour installer automatiquement, ou **non** pour revenir au menu.\n');
            break;
        }
        ...
        case 'CODE_APP_INIT_CONFIRM': {
            ...
            stream.markdown('❓ Répondez **oui** pour initialiser le projet, ou **non** pour revenir au menu.\n');
            break;
        }
        ...
        case 'CONNEXION_CHOIX_COMPTE': {
            ...
            if (messageNormalise === '0') {
                ...
                stream.markdown('> Un navigateur va s\'ouvrir pour vous authentifier sur Power Platform.\n\n');
                executerDansTerminal('pac auth create');
                stream.markdown('---\n\n');
                stream.markdown('Une fois l\'authentification terminée dans le navigateur, tapez **suite** pour voir la liste des environnements disponibles.\n');
                break;
            }
            ...
            if (!isNaN(indexCompte) && indexCompte > 0) {
                ...
                stream.markdown('> Récupération de la liste des environnements accessibles...\n\n');
                ...
                stream.markdown('> Tapez l\'**URL** ou l\'**ID** de l\'environnement (visible dans le terminal), ou `menu` pour revenir.\n');
                break;
            }

            stream.markdown('❓ Saisie non reconnue. Tapez un **numéro de compte**, `0` pour ajouter un compte, ou `menu` pour revenir.\n');
            break;
        }
        ...
        case 'CONNEXION_AJOUT_COMPTE': {
            ...
            if (messageNormalise === 'suite' || messageNormalise === 'continuer') {
                stream.markdown('✅ **Compte ajouté.** Récupération de la liste des environnements...\n\n');
                executerDansTerminal('pac env list');
                session.etat = 'CONNEXION_CHOIX_ENV';
                stream.markdown('---\n\n');
                stream.markdown('**Quel environnement souhaitez-vous utiliser ?**\n\n');
            ...
            stream.markdown('⏳ Une fois l\'authentification terminée dans le navigateur, tapez **suite** pour continuer.\n');
            break;
        }
        ...
        case 'CONNEXION_CHOIX_ENV': {
            ...
            if (sauvegarde) {
                stream.markdown('> 💾 Environnement sauvegardé dans `power.config.json`.\n\n');
            } else {
                stream.markdown('> ℹ️ Aucun fichier `power.config.json` trouvé — l\'environnement n\'a pas été sauvegardé localement.\n\n');
            }

            stream.markdown('---\n\n');
            stream.markdown('✅ **Connexion et environnement configurés.** Retour au menu principal.\n\n');
            reinitialiserSession(threadId);
            afficherMenuPrincipal(stream);
            break;
        }
        ...
        case 'CODE_APP_INIT_ENV': {
            ...
            stream.markdown('1. 🔐 Authentification sur l\'environnement Power Platform\n');
            stream.markdown('2. 📦 Installation du SDK **@microsoft/power-apps**\n');
            stream.markdown('3. 🔧 Initialisation du projet Code App\n\n');
            stream.markdown('> ✅ Suivez les instructions dans le terminal. Une fois terminé, utilisez **3** (`run`) pour démarrer le proxy.\n\n');
...
function afficherMenuPrincipal(stream: vscode.ChatResponseStream): void {
    ...
    stream.markdown('> Votre assistant de déploiement Power Platform. Il vous guide pas à pas pour exporter, configurer et importer vos solutions Dataverse entre environnements, directement depuis VS Code.\n\n');
    stream.markdown('---\n\n');
    stream.markdown('## Menu principal\n\n');
    stream.markdown('| # | Action | Description |\n');
    stream.markdown('|---|--------|-------------|\n');
    stream.markdown('| **1** | `init` | Initialiser un projet front-end (Vue · React · Angular) avec la stack Expertime |\n');
    stream.markdown('| **2** | `codeapp` | Transformer le projet en Power Apps Code App |\n');
    stream.markdown('| **3** | `run` | Démarrer proxy & serveur local |\n');
    stream.markdown('| **4** | `push` | Pousser le Code App (Inner-loop) |\n');
    stream.markdown('| **5** | `connexion` | Gérer la connexion et sélectionner l\'environnement Power Platform |\n\n');
}

export function activate(context: vscode.ExtensionContext) {
    const participant = vscode.chat.createChatParticipant('expertees-deploy.agent', gererRequete);
